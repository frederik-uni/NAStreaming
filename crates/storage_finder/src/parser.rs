...
fn parse_episodes(
    ...
    for entry in get_valid_files(p) {
        ...
        let pro_name = entry
            ...
            .to_lowercase();
        let mut ftype = FileType::try_from(pro_name.as_str()).expect("Filtered before");
        let file_name = pro_name.clone();
        let mut temp = pro_name.split('.').collect::<Vec<_>>();
        temp.pop();
        ...
        if let Some(captures) = ep_regex.captures(&pro_name.clone()) {
