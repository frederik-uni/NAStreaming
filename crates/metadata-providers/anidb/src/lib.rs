...
use metadata_provider::{fetcher::Url, Issue, MetadataProvider};
...
impl Instance {
    pub fn new(
        ...
        Ok(Box::new(Self { server: "http://api.anidb.net:9001/httpapi?request=anime&client=nastreamingclien&clientver=2&protover=1".parse().expect("Cant fail"),
...
mod tests {
    ...
    async fn demo() {
        ...
        let result = search_instance
            .search(&Client::new(), "One piece", Some(1992), None)
            .await
            .expect("Test failed");
